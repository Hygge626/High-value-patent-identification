...
log_file = f"logs/修复_batch_size_mismatch_{timestamp}.log"
...
def main(args):
    ...
    for k in ['loss', 'acc', 'precision', 'recall', 'f1', 'auc']:
        ...
        std  = arr.std(ddof=0) if arr.size > 0 else 0.0  # 总体标准差；如需无偏用 ddof=1
...
if __name__ == "__main__":
    ...
    kv = load_sgns("D:/CNNATTGAT/Public-data/numberbatch-en-19.08-plain.txt")  # 你的词向量路径
    embedder = StaticDeltaEmbedding(kv)
    ...
    # ==== 关键补丁：将每种节点的 'h' 映射为统一的 'feat' ====
    for ntype in g.ntypes:
        # 如果已有 feat 就跳过；优先使用已有的 h
        ...
        if 'h' in g.nodes[ntype].data:
            ...
            raise RuntimeError(f"[{ntype}] 缺少 'h' 特征，无法创建统一的 'feat'。请检查构图流程。")
    ...
    print("✅ 图已成功保存为 graph_with_feat.bin（包含统一的 'feat' 特征）")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_tsv',      required=True, help="路径到 g_cpc_title.tsv")
    parser.add_argument('--vector_path',    required=True, help="SGNS 词向量（word2vec 格式）")
