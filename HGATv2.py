...
# -----------------------------------------------------------------------------
# 2. 文本编码：创新TextCNN + Self-Attention 池化
# -----------------------------------------------------------------------------
class CNNAttentionEncoder(nn.Module):
    """多核TextCNN + Self-Attention 池化 + 可动态扩展的学习型位置编码"""
    ...
    def forward(self, embeds: torch.Tensor) -> torch.Tensor:
        ...
        """
        L, D = embeds.shape  # L = 实际序列长度, D = emb_dim
        # 如果实际长度超出 max_len，就在后面补零位置编码
        ...
        # 4) 拼接成 Attention 输入 -> [1, num_kernels, channels]
        seq = torch.stack(pools, dim=1)
        # 5) Self-Attention 池化
        attn_out, _ = self.attention(seq, seq, seq)
        out = attn_out.mean(1)  # [1, channels]
        return out.squeeze(0)   # [channels]

# -----------------------------------------------------------------------------
# 3. 构建异构图：含反向边 + TF-IDF/BM25特征 + Inter-Document 边 + IPC知识图谱
# -----------------------------------------------------------------------------
class PatentGraphBuilder:
    ...
    def build(self, df: pd.DataFrame):
        # 预计算 TF-IDF 和 BM25
        abstracts = df['摘要'].fillna('').astype(str).tolist()
        ...
        for i, (_, row) in enumerate(df.iterrows()):
            ...
            # connect claim/efficacy to word nodes
...
def main(args):
    ...
    df = pd.read_excel(args.train_xlsx)[['合享价值度','IPC','标题','摘要','独立权利要求','技术功效句']]
